    ...
    def split(self, nffg, split_flows):
        ...
        if len(domains_dict) == 1:
            ...
            for split_flow in split_flows:
                print("debug")
                start_element = split_flow.split('/')[0]
                end_element = split_flow.split('/')[1]
                start_domain = split_flows[split_flow].items()[0][0].split('/')[0]
                end_element = split_flows[split_flow].items()[0][-1].split('/')[1]
                start_domain_nffg = domain_to_nffg[start_domain]
                end_domain_nffg = domain_to_nffg[end_element]

                # characterize and generate additional sub-graphs
                for i, (domain_connection, vc) in split_flow[split_flow]:
                    domain1 = domain_connection.split('/')[0]
                    domain2 = domain_connection.split('/')[1]
                    reverse_flow_id = split_flow.split('/')[1] + '/' + split_flow.split('/')[0]
                    if i == 0:
                        # uniform the generated end-point name
                        domain1.getAutogeneratedEndpoints(domain2)[0].name = split_flow
                        ...
                        if middle_nffg.getFlowRule(reverse_flow_id) is None:
                            NFFG_Manager(middle_nffg).addEndpointsCoupleAndFlowrules(split_flow, split_flow, reverse_flow_id)
                        else:
                            NFFG_Manager(middle_nffg).addFlowRule(reverse_flow_id, reverse_flow_id, split_flow)
                    if i == len(split_flows[split_flow])-1:
                        # uniform the generated end-point name
                        domain2.getAutogeneratedEndpoints(domain1)[0].name = reverse_flow_id

                    # characterize end-points
                    domain1_nffg = domain_to_nffg[domain1]
                    domain2_nffg = domain_to_nffg[domain2]
                    endpoint1 = domain1_nffg.getEndPoint(split_flow)
                    endpoint2 = domain2_nffg.getEndPoint(split_flow)
                    ...
                    if vc["labeling-method"] == "gre":
                        ...
                        self._characterize_endpoint(endpoint1,
                                                    Vlan(interface1.node, interface1.name,
                                                         DomainInformation().get_domain_info(start_domain).type,
                                                         interface2.node, interface2.name,
                                                         DomainInformation().get_domain_info(end_element).type,
                                                         vlan_id))
                        ...
                        raise UnsupportedLabelingMethod("Graph cannot be split because scheduled domains '" + start_domain +
                                                        "' and '" + end_element + "' have a virtual channel through the " +
